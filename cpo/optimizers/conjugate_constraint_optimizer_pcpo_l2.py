...
'''
This is PCPO with L2 norm projection, and this file is put in "rllab-master/sandbox/cpo/optimizers/"
'''
...
class ConjugateConstraintOptimizerPCPO_L2(Serializable):
    """
    ...
    def update_opt(self, loss, target, quad_leq_constraint, lin_leq_constraint, inputs, 
        ...
        constraint_term_1, constraint_value_1 = quad_leq_constraint # for trpo
        constraint_term_2, constraint_value_2 = lin_leq_constraint # for constraint
        ...
        ### This is original hessian function
        ...
        self._opt_fun = ext.lazydict( # optimization function
    ...
    def optimize(self, 
        ...
        approx_g = Hx(v)
        q = v.dot(approx_g) # approx = g^T H^{-1} g #equation (12)
        ...
        if c > 0:
            ...
            logger.record_tabular("Warning_safeyViolate", 1)
        else:
            ...
            logger.record_tabular("Warning_safeyViolate", 0)
        ...
        ###############################################################################################
        ## Here, I modified the policy update direction
    
        ### Trust region projected constrainted policy optimization ###
        ### The following piece of the code is based on L2 projection ###
        ...
        logger.record_tabular("beta_soft",beta_soft)
        logger.record_tabular("proj_thre_percent",0)
        ###############################################################################################
        if optim_case >= 0:
            ...
            flat_descent_step = flat_descent_step_tr + beta_soft * flat_descent_step_pg
            # store the Lagrangain multiplier

            logger.record_tabular("pcpo_reward",np.sqrt(delta / (q + eps)))
            logger.record_tabular("pcpo_cost",lam_pg)
            logger.record_tabular("pcpo_cost_scaledByBeta", beta_soft *lam_pg)
            
        else: 
            # when infeasible, take the step that purly decreases the constrained value
            flat_descent_step = np.sqrt(delta / (s + eps)) * w
            # store the Lagrangain multiplier
            logger.record_tabular("pcpo_reward",0)
            logger.record_tabular("pcpo_cost",np.sqrt(delta / (s + eps)))
            logger.record_tabular("pcpo_cost_scaledByBeta", 0)
        ...
        if precomputed_threshold is not None: # adject the threshold
            lin_reject_threshold = precomputed_threshold
        if diff_threshold:
            lin_reject_threshold += prev_lin_constraint_val
        logger.record_tabular("LinRejectThreshold",lin_reject_threshold) # ture threshold
        ...
        def line_search(check_loss=True, check_quad=True, check_lin=True):
            ...
            logger.record_tabular("LinConstraintVal", lin_constraint_val)
            logger.record_tabular("QuadConstraintVal", quad_constraint_val)
        ...
        def wrap_up():
            if optim_case < 4:
                ...
                next_linear_S = S + flat_b.dot(cur_param - prev_param) # linear approximation (equation 11)
                next_surrogate_S = S + lin_constraint_delta # ture error
        ...
        if optim_case == 1 and not(self.revert_to_last_safe_point):
            ...
            wrap_up()
            return
        elif optim_case == -1: # TRPC
            # store the parameter
            self._target.set_param_values(prev_param - flat_descent_step, trainable=True)
            # store the parameter for checking the zig zag 
            self.parab_list.append(-flat_descent_step) #\pi^{k+1} - \pi^{k}
            logger.record_tabular("Parab_L2norm", np.linalg.norm(-flat_descent_step))
            # check the size of the list
            if len(self.parab_list) >= 2:
                old_p = np.array(self.parab_list[-2])
                new_p = np.array(self.parab_list[-1])
                angle_pi = angle_between(old_p, new_p)
                degree_360 = np.degrees(angle_pi)
                logger.record_tabular("Parab_angle(pi)", angle_pi)
                logger.record_tabular("Parab_degree(360)", degree_360)
            else: # first iteration
                logger.record_tabular("Parab_angle(pi)", float('nan'))
                logger.record_tabular("Parab_degree(360)", float('nan'))
        ...
        if optim_case != -1: # CPO + our approaches
